...
namespace BistroService.Controllers
{
    ...
    public class AdminController : ApiController
    {
      ...
      // [Logged]
        ...
        public HttpResponseMessage InsertProduct(ProductDTO product)
        {
           ...
           // DeliveryManDTO addproduct = new DeliveryManDTO();
            string senderEmail = "BistroBD@gmail.com";
            string senderPassword = "oulcgsufebduckez";
            string recipientEmail = ""; // your mail account 
            string subject = "Welcome to Our BIStroBoss resturant ";
            string body = "<div style=\"font-family: Arial, sans-serif; font-size: 14px; color: #0d0d0d; font-weight: 300;\">"
                  + "<h4>Dear " + admin.Name + ",</h4>"
                  + "<p>Thank you for adding product in Bistro Boss resturant . We are thrilled to have you ! " +
                  ...
                  + "<p>Once again, welcome to Bistro Boss resutrant, and thank you for choosing us.</p>"
                  + "<h4>Best regards,</h4>"
                  + "<h3>Bistro Boss  </h3>"
                  + "</div>";
     ...
     //   [Logged]
        ...
        public HttpResponseMessage FoodCount()
        {
            try
            {
                var data = ProductService.GetProduct().Count;
                return Request.CreateResponse(HttpStatusCode.OK, data);
            }
            catch (Exception ex)
            {
